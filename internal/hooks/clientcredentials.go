...
func (c *clientCredentialsHook) AfterError(ctx AfterErrorContext, res *http.Response, err error) (*http.Response, error) {
	...
	if res != nil && res.StatusCode == http.StatusUnauthorized {
		fmt.Println("afterError", res.StatusCode, "delete cache")
...
func (c *clientCredentialsHook) doTokenRequest(ctx HookContext, credentials *credentials, scopes []string) (*session, error) {
	// fmt.Println("doTokenRequest", credentials.ClientID, credentials.ClientSecret, credentials.Resource, scopes)
	values := url.Values{}
	...
	values.Set("resource", credentials.Resource)
	// values.Set("scope", strings.Join(scopes, " "))

	authToken := base64.StdEncoding.EncodeToString([]byte(credentials.ClientID + ":" + credentials.ClientSecret))
	...
	// fmt.Println("doTokenRequest", tokenURL, values.Encode())
...
func (c *clientCredentialsHook) getSession(ctx BeforeRequestContext, credentials *credentials) (*session, error) {
	...
	// fmt.Println("getSession", sessionKey, rawCachedSession, ok)
	...
	rawSession, err, _ := c.sessionsGroup.Do(sessionKey, func() (any, error) {
		// fmt.Println("doTokenRequest", sessionKey)
