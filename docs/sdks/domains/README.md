...
func main() {
    ...
    res, err := s.Domains.Create(ctx, operations.CreateDomainRequest{
        Domain: "thin-disappointment.org",
    })
    if err != nil {
        log.Fatal(err)
    }
...
func main() {
    ...
    res, err := s.Domains.Delete(ctx, "<id>")
    if err != nil {
        log.Fatal(err)
    }
    if res != nil {
